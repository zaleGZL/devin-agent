...
describe("permission vendor contract", () => {
  ...
  it("preserves standard permission options and emits the verified update key", () => {
    ...
    expect(permissionDecisionFromInteraction({ action: "select", optionId: "allow_once", updatedCommand: "touch edited.txt" }, options)).toEqual({
      outcome: { outcome: "selected", optionId: "allow_once" },
      _meta: { "cognition.ai/updatedInput": "touch edited.txt" },
    });
