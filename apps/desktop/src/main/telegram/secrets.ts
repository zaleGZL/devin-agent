...
export interface TelegramSecretsData {
  botToken?: string;
}
...
export class TelegramSecrets {
  ...
  async read(): Promise<TelegramSecretsData> {
    ...
    if (typeof envelope.encrypted === "string") {
      ...
      try {
        return JSON.parse(cipher.decrypt(Buffer.from(envelope.encrypted, "base64"))) as TelegramSecretsData;
      } catch {
        return {};
      }
    }
  ...
  async write(data: TelegramSecretsData): Promise<void> {
...
function normalizeSecrets(value: unknown): TelegramSecretsData {
  if (!value || typeof value !== "object") return {};
  const data = value as Record<string, unknown>;
  return {
    ...(typeof data.botToken === "string" && data.botToken ? { botToken: data.botToken } : {}),
  };
}
